...
function Keyword() {
    ...
    useEffect(() => {
        const fetchKeywords = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`${SubFastAPI}/api/topic/keywordExtract?pdf_link=${pdf_link}`);
    ...
    useEffect(() => {
        const fetchSummary = async () => {
            if (title && !summaryFetched) {
                setLoading(true);
                try {
                    const response = await axios.get(`${SubFastAPI}/api/summary/summaryPaper?pdf_link=${pdf_link}`);
